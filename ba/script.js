...
function showCurrencyNotification(currency, symbol) {
    currencyText.textContent = `Генерация Сигнала для ${currency} ${symbol}`;
...
async function startSignalGeneration(currency, symbol) {
    ...
    console.log(`Запрос сигнала для валюты: ${currency} (${symbol})`);
    ...
    setTimeout(() => {
        ...
        console.log(`Результат для ${currency}: ${targetMultiplier}x`);
