document.addEventListener("DOMContentLoaded", () => {
    ...
    function displayRandomMessage() {
        const messages = ["ПРАВО", "ЦЕНТР", "ЛЕВО"]; const skullIds = ["skull3", "skull2", "skull1"]; const randomIndex = Math.floor(Math.random() * messages.length);
    ...
    document.addEventListener("click", function (event) {
        if (event.target.matches("button[data-link]")) { window.location.href = "https://fiozatj.github.io/luckytajik/"; }
    });
